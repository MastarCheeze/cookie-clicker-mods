const mod = {
    init: function () {
        ...
        Game.ClickCookie = (function (oldFunc) {
            return function (e, amount, autoClicker) {
                if (!mod.prefs.autoClicker || autoClicker) return oldFunc.apply(this, arguments);
            };
        })(Game.ClickCookie);

        if (!Game.mods.CookieMonster) { // Cookie monster already overrides big cookie click event
        ...
        Game.tickerL.addEventListener("mouseover", () => {
            if (Game.TickerEffect && Game.TickerEffect.type === "fortune" && mod.prefs.clickFortune) Game.tickerL.click();
        });
        ...
        Game.UpdateMenu = () => {
            ...
            if (Game.onMenu === "prefs") {
                ...
                for (let i = 0; i < blocks.length; i++) if (blocks[i].textContent.search(loc("Settings")) === 0) settingsBlock = blocks[i];
                ...
                const optionsHTML = `
                    <div class="block" style="padding: 0px; margin: 8px 4px;">
                      <div class="subsection" style="padding: 0px;">
                        ...
                        <div class="listing">
                          ${this.writePrefButton("autoClicker", "autoClickerButton", "Auto Clicker" + ON, "Auto Clicker" + OFF, "")}<label>(click big cookie on hover)</label><br>
                        </div>
                        <div class="listing">
                          ${this.writeSlider("autoClickerCpsSlider", "Auto Clicker CPS", "[$] CPS", () => { return this.prefs.autoClickerCps; }, 5, 20, 0.1, "Game.mods.Hoverclicker.prefs.autoClickerCps=Math.round(l('autoClickerCpsSlider').value*100)/100;Game.mods.Hoverclicker.updateAutoClicker();l('autoClickerCpsSliderRightText').innerHTML=Game.mods.Hoverclicker.prefs.autoClickerCps+' CPS';")}<br>
                        </div>
                        <div class="listing">
                          ${this.writePrefButton("clickGC", "clickGCButton", "Click golden cookies" + ON, "Click golden cookies" + OFF, "")}<label>(click golden cookies on hover)</label><br>
                        </div>
                        <div class="listing">
                          ${this.writePrefButton("clickWrath", "clickWrathButton", "Click wrath cookies" + ON, "Click wrath cookies" + OFF, "")}<label>(click wrath cookies on hover, must have click golden cookies on hover option turned on)</label><br>
                        </div>
                        <div class="listing">
                          ${this.writePrefButton("clickWrinkler", "clickWrinklerButton", "Click wrinklers" + ON, "Click wrinklers" + OFF, "")}<label>(pop wrinklers on hover when they reach the big cookie)</label><br>
                        </div>
                        <div class="listing">
                          ${this.writePrefButton("clickFortune", "clickFortuneButton", "Click fortune cookies" + ON, "Click fortune cookies" + OFF, "")}<label>(click news ticker fortunes on hover)</label><br>
                        </div>
                      </div>
                    </div>
                `;
    ...
    writePrefButton: function (prefName, button, on, off, callback, invert) {
        ...
        return `
            <a class="smallFancyButton prefButton option${(this.prefs[prefName] ^ invert) ? '' : ' off'}" id="${button}" ${Game.clickStr}="Game.mods.Hoverclicker.toggle('${prefName}','${button}','${on}','${off}','${invert}');${callback}">
    ...
    writeSlider: function (slider, leftText, rightText, startValueFunction, min, max, step, callback) {
        ...
        return `
            <div class="sliderBox">
              ...
              <div style="float:right;" class="smallFancyButton" id="${slider}RightText">${rightText.replace('[$]', startValueFunction())}</div>
